...
class TestLogger(unittest.TestCase):
    ...
    def test_log_daily_question(self):
        ...
        question_from_db = self.db.execute_query("SELECT * FROM questions WHERE question_text = 'Is this a test?'")
    ...
    def test_log_player_guess(self):
        ...
        self.logger.log_player_guess("player1", "PlayerOne", daily_q['id'], "My Answer", True)
        
        guess_from_db = self.db.execute_query("SELECT * FROM guesses WHERE player_id = 'player1'")
    ...
    def test_log_messaging_event(self):
        ...
        message_from_db = self.db.execute_query("SELECT * FROM messages WHERE recipient_sender = '12345'")
    ...
    def test_read_guess_history(self):
        ...
        daily_q1_id = self.db.execute_query("SELECT id FROM daily_questions ORDER BY id DESC LIMIT 1")[0]['id']
        self.logger.log_daily_question(q2, [])
        daily_q2_id = self.db.execute_query("SELECT id FROM daily_questions ORDER BY id DESC LIMIT 1")[0]['id']
